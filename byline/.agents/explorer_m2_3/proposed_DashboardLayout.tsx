...
export function DashboardLayout({ onLandingClick }: DashboardLayoutProps) {
  ...
  // Milestone 2: Active query being processed by pipeline simulation
  const [runQuery, setRunQuery] = useState<string | null>(null);

  // Milestone 2: Heading ID to scroll to when navigating to Docs tab
  const [docsScrollTarget, setDocsScrollTarget] = useState<string | null>(null);

  // Projects list and active project states
  ...
  // Milestone 2: Modified runPipeline to accept query parameter
  const runPipeline = (query?: string) => {
    ...
    if (query) {
      setRunQuery(query);
      console.log(`[Pipeline Run] Initiating dispatch for milestone: "${query}"`);
    } else {
      setRunQuery(null);
    }
  ...
  // Milestone 2: Navigation Destinations & Indexed Docs Headings
  const commands = [
    // Navigation Group
    {
      id: "nav-overview",
    ...
    // Documentation Group (Indexed headings from DocsTab)
    {
    ...
    <div style={{ height: "100vh", display: "flex", flexDirection: "column", background: "var(--by-bg)", color: "var(--by-text)", overflow: "hidden" }}>
      ...
      <div style={{ flex: 1, display: "flex", overflow: "hidden", position: "relative" }}>
        ...
        <RunLogPanel
          ...
          // Optionally pass runQuery to display the active query in the UI logs
        />
