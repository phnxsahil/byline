...
const SECTIONS = [
  ...
  { id: "dispatching-from-the-dashboard", title: "Dispatching from the Dashboard", content: `Type a milestone into Overview's new milestone box, the chat/rail composer, or the Cmd+K palette, and all three paths trigger the same dispatch flow.\n\nThe dashboard sends your milestone to the Byline API, which runs the full agent pipeline and returns drafts for all selected platforms.\n\nA CLI is planned once the dispatch API is public — it'll be a thin wrapper around the same endpoint these three paths already call.` },
